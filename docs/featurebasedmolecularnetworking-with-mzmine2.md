...
A separate command line tool [parse_mgf_by_feature_ID](https://github.com/Zquinlan/Python_scripts/blob/master/README.md) allows you to reduce the number of features in your mgf file based off a second text file containing the feature ID's. Simply have to define the file feature ID .txt file, the mgf file (with a .txt extension) and the exported mgf txt file name. (e.g. python3 parse_mgf_by_feature_ID.py feature_ids.txt input_mgf.txt > outfile.txt)
