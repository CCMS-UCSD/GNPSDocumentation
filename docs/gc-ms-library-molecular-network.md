...
(1), (2) The .mgf file containing the deconvolved EI spectra and the .csv feature quantification table automatically (with balance score) is attached if the spectral library search is launched from the deconvolution job performed with MSHub. Otherwise, they can be added by the user from other processing software. The quantification table is formatted as the MZmine2 output table with a "raw ID", "row m/z", "row retention time" columns, and all the peak area values for all .mzML files.
...
(4) Public and open-source libraries files are selected by default. However, user could add their libraries on GNPS by following the instructions [here](batchupload.md), or by uploading them in .mgf format (drag and drop on “Upload File” section). 
...
The **job window** contains four fields of: advanced search, filtering, network, and Kovats index calculation options. Click on the “Show Fields” button to expand the options. The default parameters recommended for obtaining good results have been determined and pre-populated as described in the table Parameters for Library Search. However, users are encouraged to adjust the settings as appropriate for their analysis. 
...
| Top Hits Per Spectrum                 | How many top hits ranked by the cosine score to be included into the results. | The default of 1. It is recommended to keep this number relatively high (for example, 10 to include top 10 matches) to allow for inclusion of possible correct annotation selection if it is not a top hit. |
...
| ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **Fillable Field**                                           | **Definition**                                               | **Recommended User Input**                                   |
| Perform Kovats Calculation                                   | Whether to perform calculation of Kovats retention index.    | If the user need to calculate Kovats RIs for the compounds during library search this option should be selected (optional). (i) The Input Carbon Marker File could be provided for Kovats markers, but it must be conducted with identical protocol as the rest of the experiment. (ii) The Carbon Marker File is not provided, so the Kovat’s RIs will be estimated based on dataset-wide annotations by performing a polynomial fitting of  Kovats RIs values of high-quality annotations (experimental feature, may not be suitable for all datasets). For the estimation, the values for other field in the drop-down menu need to be provided. |
...
After the job has been completed, the **job status page** gives access to several views:
...
(vi) The link “View Kovats Calculation Result” shows Kovats calculation results for each spectrum if selected when launching the job. If that option was not selected or a hydrocarbons markers reference file was not provided (or provided in an incorrect format), a message: "There was an error retrieving the result data for block "main" of workflow type "MOLECULAR-LIBRARYSEARCH-GC"" will be displayed instead.
...
GNPS infrastructure enables molecular networking, which represents similar mass spectra as connected nodes in a network. Based on the premise that similar molecular structures would generate similar fragments, the network could be used to propagate structural information between nodes/compounds within the network  and gain a better perspective on which annotations may be erroneous (the network of GC-MS data can be used to point to the correct molecular family, even when the top annotation may be incorrect). The networks can also be used for visualizing chemical distributions and propagate chemical ontology information within the network.  The networking is not limited to a single study, but can be extended to any number of datasets once deconvolution has been carried out. 
